...
const Task = forwardRef(({ id, notes, index, removeTask }, ref) => {
  ...
  // Contains the logic to delete the task from the Datastore
  const deleteTask = useCallback(() => {
    setDeleting(true);
    axios
      .delete(`/server/to_do_list_function/${id}`) // Ensure that 'to_do_list_function' is the correct endpoint
      .then((response) => {
        const {
          data: {
            todoItem: { id },
    ...
    if (fetchState !== 'fetched') {
      axios
        .get('/server/to_do_list_function/all', {
          params: { page, perPage: 200 }, // Parameters contain the start and end limits of data (tasks)
        })
        .then((response) => {
          const {
          ...
          if (page === 1) {
            ...
            setTodoItems((prev) =>
              [...new Map([...prev, ...todoItems].map((item) => [item.id, item]))].map(
                (item) => item[1]
              )
            );
          }
        ...
        <>
          ...
          <div className="create-container">
            <form className="dF aI-center w-full" onSubmit={createTodo}>
              ...
              >
                Create Task
                {submitting && <div className="btn-primary__loader ml-5"></div>}
              </button>
            </form>
          </div>
